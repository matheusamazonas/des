...
void setColumnValues(lightInfo* column, int on)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (column[i].value)
        {
            ...
            writeToPin(pinFd, on);
            //printf("Writing %d on pin %d\n", column[i].value, column[i].pin);
        }
    }
}
...
void periodicLeft(void* arg)
{
	...
	while (1) 
	{
		...
		{
		    period = pRight + pLeft;
			cDelay = (8 * cCount * (period/N))/2;
            rt_task_sleep(pLeft/2L + period/4L - tError - cDelay);
			for (j = 7; j >= 0; j--)
            {
...
void periodicRight(void* arg)
{
	...
	while(1)
	{
		...
		if (toRight)
		{
		    period = pRight + pLeft;
			cDelay = (8 * cCount * (period/N))/2;
            rt_task_sleep(pRight/2L + period/4L - tError - cDelay);
			for (j = 0; j < 8; j++)
            {
                setColumnValues(matX[j], 1);
                rt_task_sleep(period/N);
